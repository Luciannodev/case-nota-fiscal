...
- `core/model` contém os modelos do domínio;
- `core/usecase` contém a porta de entrada e a implementação de cada caso de uso;
- `core/usecase/calculo` contém operações de negócio usadas pelo caso de uso;
- `core/strategy` contém as estratégias tributárias e de frete;
...
- `config/CoreConfig` é o composition root que converte propriedades externas em valores do core e monta as dependências.

O core não recebe `TaxasProperties`: recebe `BigDecimal` e `FaixasAliquota`, tipos independentes do framework. Um teste arquitetural impede imports de Spring, configuração ou adapters dentro de `core`.
...
- A composição explícita adiciona código em `CoreConfig`, em troca de dependências visíveis e controladas.
